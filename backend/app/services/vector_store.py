...
class MongoDBVectorStore:
    """In-memory stub mimicking MongoDB vector search collection."""
    ...
    async def connect(self) -> None:
        """Pretend to establish a connection to MongoDB."""

        logger.info(
            "[stub] Connecting to MongoDB at %s/%s.%s",
            self.uri,
            self.database,
            self.collection,
        )
